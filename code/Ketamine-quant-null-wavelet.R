...
# Calculate quant offset using standard
quant_offset <- calculate_quant_offset(
    combined_data, "sciex", chromatogram_data, "Quant", "STD"
)
...
# Electronic noise from SCIEX solvent blank (ROS estimates):
blank_noise_mean <- 1433.077
blank_noise_std <- 457.7373

# Load chemical noise statistics (from Ketamine-quant-chemical-noise.R)
peak_data <- read_csv(
    file.path("results", "tables", "Ketamine_quant_peak_data_sciex.csv")
)
...
cat("sciex: lambda_events =", round(lambda_events, 2),
...
write(json_text, file = file.path("results", "json", "Ketamine_quant_results.json"))
