...
instrument_data <- list(
  "6500-MW" = list(
    file_list = c(
      "Mary_RvT4_in_IA_aorta_MS4-MF-blank (5).mzML",
    ...
    sample_list = c(
      "6500-MW Blank", "6500-MW STD",
      "Chow 1", "Chow 5", "Chow 6",
      "WD 07", "WD09", "WD 10"
    ),
    quant_index = 59,
    is_index = 67,
    is_rt = 0,
    offset_ref_sample = "6500-MW STD" # Sample used for RT offset calculation
  ),
  "6500-JD" = list(
    file_list = c(
      "JD_ACP_MW_ART_051822-Blank.mzML",
    ...
    is_index = 67,
    is_rt = 0,
    offset_ref_sample = "WD 5" # Sample used for RT offset calculation (no STD available)
  )
)
...
# Calculate quant offsets using specified reference sample
calculate_quant_offset <- function(data, instr_name, ref_sample) {
  quant_rt <- chromatogram_data |>
    filter(instrument == instr_name, Sample == ref_sample, Compound == "Quant") |>
    filter(RT < 10.6) |>
    slice(which.max(Intensity)) |>
    pull(RT)

  is_rt <- data |>
    filter(instrument == instr_name, sample == ref_sample) |>
    pull(is_rt)

  if (length(quant_rt) == 0 || length(is_rt) == 0) {
    warning(paste("No reference sample found for instrument:", instr_name, "sample:", ref_sample))
    return(NA_real_)
  }
...
quant_offset_6500_MW <- calculate_quant_offset(combined_data, "6500-MW", instrument_data[["6500-MW"]]$offset_ref_sample)
quant_offset_6500_JD <- calculate_quant_offset(combined_data, "6500-JD", instrument_data[["6500-JD"]]$offset_ref_sample)
...
for (i in 1:nrow(combined_data)) {
  ...
  if (instrument_name == "6500-MW") {
    # Data from the 6500-MW blank sample:
    # Mean = 197.3
    # SD = 192.4489

    blank_noise_mean <- 197.3
    blank_noise_std <- 192.4489

    num_existing_events <- 2 # Number of existing events to sample
    ...
    peak_data <- peak_data |>
      filter(!is.na(time), !is.na(power), !is.na(scale))
  ...
  } else if (instrument_name == "6500-JD") {
    # Data from the 6500-JD blank sample
    # Mean = 545.9667
    # SD = 323.9205

    blank_noise_mean <- 545.9667
    blank_noise_std <- 323.9205

    num_existing_events <- 7 # Number of existing events to sample
    num_new_events <- 1 # Number of new chemical noise events to generate
