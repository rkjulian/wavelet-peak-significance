...
for (i in 1:nrow(combined_data)) {
  ...
  if (instrument_name == "6500") {
    # Data from the 6500 blank sample (qual ion)
    # Mean = 629.5484
    # SD = 374.79
    # Shapiro-Wilk p-value 0.2826
    # Bootstrap SD bias: -5.230671
    # Blank number of events (mean) 5.9 (sd ~ 0.64)

    blank_noise_mean <- 629.5484
    blank_noise_std <- 374.79

    num_existing_events <- 6 # Number of existing events to sample
    ...
    # Data from the 7500 blank sample (qual ion)
    # W = 0.96307, p-value = 0.06277
    # Mean = 1828.311
    # SD = 277.3259
    # Shapiro-Wilk p-value 0.16
    #
    # Blank number of events (mean) 8 (sd ~ 0.63)

    blank_noise_mean <- 1828.311
    blank_noise_std <- 277.3259

    num_existing_events <- 8 # Number of existing events to sample
    num_new_events <- 1 # Number of new chemical noise events to generate
