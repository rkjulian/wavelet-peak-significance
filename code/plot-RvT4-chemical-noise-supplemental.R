# RvT4 Chemical Noise Supplemental Plots (plot-only)
#
# Reads pre-computed results from RvT4_chemical_noise.json and generates the
# multi-page supplemental PDF. Produces the same output as RvT4-chemical-noise.R
# without re-running the CWT analysis.
...
cat("Loading pre-computed results from RvT4_chemical_noise.json...\n")
...
pdf(file.path("figures", "supplemental_figures",
              "RvT4_Chemical_Noise_Analysis.pdf"),
    width = 8.5, height = 11)

# Track instrument for summary plots at boundaries
...
for (i in seq_along(json_results)) {
    ...
    # Create chromatogram data frame
    ...
    plot1 <- chromatogram_data %>%
        ...
        ggtitle(label = bquote(
            "RvT4 Quant (m/z 361.1" %->% "211.1): " ~
            .(sample_name) ~ " (" * .(instrument_name) * ")")) +
        theme(strip.text.x = element_text(size = 12, face = "bold"))

    # Add vertical lines for reference RTs
...
# Print summary plots for the final instrument
print_summary_plots(current_instrument, all_peak_data,
    csv_prefix = "RvT4_peak_data_",
    compound_name = "Quant",
    precursor_mz = "361.1", product_mz = "211.1")
...
cat("  figures/supplemental_figures/RvT4_Chemical_Noise_Analysis.pdf\n")
