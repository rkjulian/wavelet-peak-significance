...
create_instrument_data <- function(instrument, file_list, sample_list, quant_index, is_index, is_rt) {
  tibble(
...
process_srm_data <- function(row, data_dir) {
  ...
  compounds <- c("Quant", "IS")
  indices <- c(row$quant_index, row$is_index)
...
# Mary_RvT4_in_IA_aorta_MS4-MF-9-WTWD-vehicle.mzML was excluded because it has
# No internal standard to reference for quant RT
instrument_data <- list(
  "6500-MW" = list(
    file_list = c(
      "Mary_RvT4_in_IA_aorta_MS4-MF-std mix.mzML",
      "Mary_RvT4_in_IA_aorta_MS4-MF-1-WTchow-vehicle.mzML",
      "Mary_RvT4_in_IA_aorta_MS4-MF-5-WTchow-vehicle.mzML",
      "Mary_RvT4_in_IA_aorta_MS4-MF-6-WTchow-vehicle.mzML",
      "Mary_RvT4_in_IA_aorta_MS4-MF-7-WTWD-vehicle.mzML",
      #           "Mary_RvT4_in_IA_aorta_MS4-MF-9-WTWD-vehicle.mzML",
      "Mary_RvT4_in_IA_aorta_MS4-MF-10-WTWD-vehicle.mzML"
    ),
    sample_list = c(
      "STD",
      "Chow 1",
      ...
      "Chow 6",
      "WD 07",
      #           "WD 09",   # This sample was excluded because it has no IS
      "WD 10"
    ),
    quant_index = 59,
    is_index = 67,
  ...
  "6500-JD" = list(
    file_list = c(
      "JD_ACP_MW_ART_051822 (3)..mzML",
      "JD_ACP_MW_ART_051822.mzML",
      "JD_ACP_MW_ART_051822 (2)..mzML",
      "JD_ACP_MW_ART_051822-3.mzML",
      "JD_ACP_MW_ART_051822-4.mzML",
      "JD_ACP_MW_ART_051822 (3)..mzML",
      "JD_ACP_MW_ART_051822-6.mzML"
    ),
    sample_list = c(
      "STD",
      "WD 1",
...
quant_offset_6500_MW <- calculate_quant_offset(combined_data, "6500-MW")
quant_offset_6500_JD <- calculate_quant_offset(combined_data, "6500-JD")

# Calculate PGD2 offset from IS (PGD2 is the second highest peak in Quant channel)
calculate_pgd2_offset <- function(data, instrument, rt_threshold) {
  # Get Quant peaks from STD sample, sorted by intensity
  quant_peaks <- chromatogram_data |>
    filter(instrument == !!instrument, Sample == "STD", Compound == "Quant") |>
    filter(RT > rt_threshold) |>
    arrange(desc(Intensity))

  # Second highest peak is PGD2
  pgd2_rt <- quant_peaks |>
    slice(1) |>
    pull(RT)
...
pgd2_offset_6500_MW <- calculate_pgd2_offset(combined_data, "6500-MW", rt_threshold = 10.6)
pgd2_offset_6500_JD <- calculate_pgd2_offset(combined_data, "6500-JD", rt_threshold = 10.5)

combined_data <- combined_data |>
  mutate(
    quant_rt = case_when(
      instrument == "6500-MW" ~ is_rt + quant_offset_6500_MW,
      instrument == "6500-JD" ~ is_rt + quant_offset_6500_JD,
      TRUE ~ NA_real_
    ),
    pgd2_rt = case_when(
      instrument == "6500-MW" ~ is_rt + pgd2_offset_6500_MW,
      instrument == "6500-JD" ~ is_rt + pgd2_offset_6500_JD,
      TRUE ~ NA_real_
    )
  ) |>
  filter(sample != "STD")
...
print_summary_plots <- function(instrument, peak_data) {
  ...
  plot3 <- ggplot(peak_data_valid, aes(x = time)) +
    ...
    labs(
      title = bquote("Time Distribution - PGE"[2] ~ "Quant (m/z 351.3" %->% "189.1) - " ~ .(instrument)),
  ...
  plot4 <- peak_data_valid |>
    ...
    labs(
      title = bquote("Peak Power Distribution - PGE"[2] ~ "Quant (m/z 351.3" %->% "189.1) - " ~ .(instrument)),
...
for (i in 1:nrow(combined_data)) {
  ...
  current_traces <- chromatogram_data |>
    ...
    mutate(Compound = fct_relevel(Compound, "IS", "Quant"))
  ...
  # Step 1: Remove Peaks Near `quant_rt` (PGE2) AND `pgd2_rt` (PGD2)
  proximity_threshold <- 0.03
  filtered_peaks <- all_peaks_df |>
    filter(abs(PeakTime - row$quant_rt) > proximity_threshold) |>
    filter(abs(PeakTime - row$pgd2_rt) > proximity_threshold)
  ...
  quant_offset <- if (row$instrument == "6500-MW") quant_offset_6500_MW else quant_offset_6500_JD
  ...
  plot1 <- current_traces |>
    ...
    ggtitle(label = bquote("PGE"[2] ~ "Quant (m/z 351.3" %->% "189.1): " ~ .(sample_name) ~ " (" * .(row$instrument) * ")")) +
  ...
  if (!is.na(row$pgd2_rt) && is.finite(row$pgd2_rt)) {
    plot1 <- plot1 + geom_vline(xintercept = row$pgd2_rt, color = "black", linetype = "dashed")
  }
  ...
  if (!is.na(row$pgd2_rt) && is.finite(row$pgd2_rt)) {
    plot2 <- plot2 + geom_vline(xintercept = row$pgd2_rt, color = "black", linetype = "dashed")
    # Add gray dashed lines for proximity threshold bounds
    plot2 <- plot2 + geom_vline(xintercept = row$pgd2_rt - proximity_threshold, color = "gray", linetype = "dashed")
    plot2 <- plot2 + geom_vline(xintercept = row$pgd2_rt + proximity_threshold, color = "gray", linetype = "dashed")
  }
  if (nrow(dominant_peaks_df) > 0 && any(!is.na(dominant_peaks_df$time))) {
    valid_times <- dominant_peaks_df$time[!is.na(dominant_peaks_df$time) & is.finite(dominant_peaks_df$time)]
  ...
  pgd2_offset <- if (row$instrument == "6500-MW") pgd2_offset_6500_MW else pgd2_offset_6500_JD
