...
# -----------------------------------------------------------------------------
# List of result JSON files to process (one per compound/transition).
# Add new compounds here as they are analyzed.
# -----------------------------------------------------------------------------

result_files <- c(
    file.path("results", "json", "Gabapentin_quant_results.json"),
    file.path("results", "json", "Clonidine_quant_results.json"),
...
for (json_file in result_files) {
    ...
    chrom_type   <- simulation_results[[1]]$chrom_type
    if (is.null(chrom_type)) chrom_type <- "Quant"

    cat("Processing", analyte_name, chrom_type, "-",
        length(simulation_results), "samples\n")

    # Derive output PDF name from analyte name and chromatogram type
    pdf_name <- paste0(analyte_name, "-", tolower(chrom_type), "-plots.pdf")
