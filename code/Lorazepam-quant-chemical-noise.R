...
# Open PDF file
pdf(file.path("figures", "supplemental_figures", "Lorazepam_Quant_Chemical_Noise_Analysis.pdf"),
...
for (i in 1:nrow(combined_data)) {
    ...
    plot1 <- plot_traces |>
        ...
        ggtitle(label = bquote("Lorazepam Quant (m/z 321.2" %->% "275): " ~ .(sample_name) ~ " (" * .(row$instrument) * ")")) +
...
write(json_text, file = file.path("results", "json", "Lorazepam_quant_chemical_noise.json"))
