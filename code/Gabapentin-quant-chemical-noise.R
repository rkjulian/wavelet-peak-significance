...
# Calculate analyte2 offset from IS (second analyte elutes after Gabapentin in Quant channel)
gabapentin_quant_rt <- chromatogram_data |>
    filter(instrument == "waters", Sample == "64X 1", Compound == "Quant") |>
    filter(RT >= 0.6, RT <= 3.0) |>
    slice(which.max(Intensity)) |>
    pull(RT)

analyte2_rt_abs <- chromatogram_data |>
    filter(instrument == "waters", Sample == "64X 1", Compound == "Quant") |>
    filter(RT > gabapentin_quant_rt + 0.05) |>
    slice(which.max(Intensity)) |>
    pull(RT)

is_rt_std <- combined_data |>
    filter(instrument == "waters", sample == "64X 1") |>
    pull(is_rt)

analyte2_offset_waters <- analyte2_rt_abs - is_rt_std

combined_data <- combined_data |>
    mutate(
        quant_rt = is_rt + quant_offset_waters,
        analyte2_rt = is_rt + analyte2_offset_waters
    )

# Open PDF file
pdf(file.path("figures", "supplemental_figures", "Gabapentin_Quant_Chemical_Noise_Analysis.pdf"),
...
for (i in 1:nrow(combined_data)) {
    ...
    # Trim to 0.6-3.0 min analysis window
    ...
    if (nrow(all_peaks_df) > 0 && "PeakTime" %in% names(all_peaks_df)) {
        ...
        if (length(significant) > 0) {
            ...
            group_summary <- tibble(
                ...
                filter(abs(time - row$analyte2_rt) > proximity_threshold)
    ...
    plot1 <- plot_traces |>
        ...
        ggtitle(label = bquote("Gabapentin Quant (m/z 172.3" %->% "154.3): " ~ .(sample_name) ~ " (" * .(row$instrument) * ")")) +
    ...
    if (!is.na(row$analyte2_rt) && is.finite(row$analyte2_rt)) {
        vline_df <- data.frame(xint = row$analyte2_rt, panel = quant_panel)
        plot1 <- plot1 + geom_vline(data = vline_df, aes(xintercept = xint),
                                     color = "black", linetype = "dashed")
    }
    ...
    if (!is.na(row$analyte2_rt) && is.finite(row$analyte2_rt)) {
        plot2 <- plot2 + geom_vline(xintercept = row$analyte2_rt, color = "black", linetype = "dashed")
        plot2 <- plot2 + geom_vline(xintercept = row$analyte2_rt - proximity_threshold, color = "gray", linetype = "dashed")
        plot2 <- plot2 + geom_vline(xintercept = row$analyte2_rt + proximity_threshold, color = "gray", linetype = "dashed")
    }
    if (nrow(dominant_peaks_df) > 0 && any(!is.na(dominant_peaks_df$time))) {
        valid_times <- dominant_peaks_df$time[!is.na(dominant_peaks_df$time) & is.finite(dominant_peaks_df$time)]
    ...
    json_results[[i]] <- list(
        ...
        analyte2_rt     = row$analyte2_rt,
        quant_offset    = quant_offset,
        analyte2_offset = analyte2_offset_waters,
...
write(json_text, file = file.path("results", "json", "Gabapentin_quant_chemical_noise.json"))
